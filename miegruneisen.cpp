...
double FEOSMieGruneisen::getG(double ro) {
	...
	double G = R/CV/M*(a0 + (1.-a0)*std::exp(-std::pow(x/.5273, 1.7)) + a1*std::exp(-std::pow(x/1.0904, -3.5)) + a2*std::exp(-std::pow(x/1.3927, -5.)));
	return G;
}
...
double FEOSMieGruneisen::getp0(double ro) {
	...
	double x = ro/ro0;
	double p0 =  x*(A*std::pow(x, -beta) * std::exp(b*(1.-std::pow(x, -beta))) - K*(std::pow(x, xi)));
	return p0;
}
...
double FEOSMieGruneisen::gete0(double ro) {
	...
	// Cold component, Born-Meyer potential
	double x = ro/ro0;
	double e0 = 1./ro0*(A/beta/b * std::exp(b*(1.-std::pow(x, -beta))) - K/xi*std::pow(x, xi));
	return e0;
}
...
double FEOSMieGruneisen::getGPrime(double ro) {
	...
	double GPrime = .035412*std::exp(-5.23948*std::pow(x, -5.))*std::pow(x, -6)
...
double FEOSMieGruneisen::getKSPrime(double ro, double e) {
	...
	double dGdx = - c0*(1.-a0)/b0*std::exp(-std::pow(x/b0, c0))*std::pow(x/b0, c0-1.) - a1*c1/b1*std::exp(-std::pow(x/b1, c1))*std::pow(x/b1, c1-1.) - a2*c2/b2*std::exp(-std::pow(x/b2, c2))*std::pow(x/b2, c2-1.);
	double dGdro = ro0*dGdx;
	// G"(ro)
	double d2Gdx2 = - c0*(1.-a0)/b0/b0*std::exp(-std::pow(x/b0, c0))*(-c0*std::pow(x/b0, 2.*c0-2.)+(c0-1.)*std::pow(x/b0, c0-2.))
	...
	double dp0dx = A*std::pow(x, -beta)*std::exp(b*(1.-std::pow(x, -beta))) - K*std::pow(x, xi) + A*beta*std::exp(b*(1.-std::pow(x, -beta)))*(-std::pow(x, -beta) + std::pow(x, -2.*beta)) - K*xi*std::pow(x, xi);
	double dp0dro = ro*dp0dx;
	double e0 = 1000./ro0*(A/beta/b * std::exp(b*(1.-std::pow(x, -beta))) - K/xi*std::pow(x, xi));
	double de0dx = 1./ro0*(A*std::exp(b*1.-std::pow(x, -beta))*std::pow(x, -beta-1.) - K*std::pow(x, xi-1));
	double de0dro = ro0*de0dx;
	double dpdro = dp0dro + (G+ro*dGdro)*(e-e0) - ro*G*de0dro;
	// p'(e)
	double dpde = ro*G;
	double d2p0dx2 = A*std::exp(b*(1.-std::pow(x, -beta)))*(-beta*(1.-beta)*std::pow(x, -beta-1.) + (b*beta-(2.+b)*beta*beta)*std::pow(x, -2.*beta-1.) + b*beta*beta*std::pow(x, -3.*beta-1.)) - K*xi*(xi+1.)*std::pow(x, xi-1.);
	double d2p0dro2 = ro0*ro0*d2p0dx2;
	double d2e0dx2 = 1./ro0*(A*std::exp(b*(1-std::pow(x, -beta)))*(b*beta*std::pow(x, 2.*beta-2.) - (beta+1.)*std::pow(x, -beta-2.)) - K*std::pow(x, xi-2.));
...
T FEOSMieGruneisenAl<T>::getp(T ro, T e) {
	...
	T p_c = B/gamma * (pow(x, gamma)-1.);
	T e_c = (B/gamma/(gamma-1.)*pow(x, gamma) - B*x/(gamma-1.) + B/gamma)/ro;
...
T FEOSMieGruneisenAl<T>::getc(T ro, T p) {
	...
	T x = ro/ro0;
	T p_c = B/gamma * (pow(x, gamma)-1.);
	T c  = sqrt((G+1)*(p-p_c)/ro + B/ro0*pow(x, gamma-1));
	return c;
}
